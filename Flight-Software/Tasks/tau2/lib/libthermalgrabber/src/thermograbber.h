...
public:
    ...
    void setSyncModeNoExternalSyncOrMaster();
    void setSyncModeCameraSlave();
    void setSyncModeGrabberMaster(uint32_t freq, uint32_t count);
...
private:
    ...
    void writeSyncHeader(uint8_t* buffer);
    void sendSyncValues(uint8_t syncMode, uint32_t time, uint32_t count);
