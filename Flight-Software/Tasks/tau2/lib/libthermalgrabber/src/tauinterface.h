...
public:
    ...
    */
    ...
    void setExternalSyncToDisabled(); // Tau 2 Core generates its own sync signal without signal output
    void setExternalSyncToMaster(); // Tau 2 Core generates its own sync signal with signal output
    void setExternalSyncToSlave(); // Tau 2 Core gets sync signal from external source (TCGrabberUSB or external sync input)

    // Settings of TCGrabberUSB
    void disableExternalSynchronization();// Tau 2 Core has to generate its sync signal by itself
    void enableExternalSynchronizationSlaveMode();// Sync signal from external sync input
    void enableExternalSynchronizationMasterMode(uint32_t freq, uint32_t count);// Sync signal generated by TCGrabberUSB

    // If watchDogCnt becomes zero, connection gets resetted
    //static unsigned int watchDogCnt;
    void reenableFTDIConnection();

    // set watchdog wait time in seconds
    void setWatchdogTime(unsigned int wait_time);
    void decrementWatchdogTime();
    unsigned int getWatchdogTime();
    unsigned int getWatchdogResetTime();
    bool getWatchdogState();
    //void setWatchdogState(bool active);
...
private:
    ...
    thermal_grabber::GainMode _gainMode;
    ...
    bool mRunWatchdog;
    unsigned int mWatchdogCnt;
    unsigned int mWatchdogCntResetValue;
};
