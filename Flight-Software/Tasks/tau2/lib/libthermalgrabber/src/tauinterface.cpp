...
void watchDog(TauInterface* instance)
{
    ...
    while (ti->getWatchdogState())
    {
        std::this_thread::sleep_for(std::chrono::milliseconds(1000));

        unsigned int wdTime = ti->getWatchdogTime();
        unsigned int wdResetTime = ti->getWatchdogResetTime();

        if (wdTime < wdResetTime && wdTime != 1)
            std::cerr << "watchdog in " << wdTime-1 << std::endl;

        if (wdTime == 1)
        {
            std::cerr << "Stopping unresponsive ThermalCapture GrabberUSB" << std::endl;
            ti->stopGrabber();
            std::cerr << "Stopping done" << std::endl;

            if (ti->threadTauConnection.joinable())
            {
                std::cerr << "Joining connection thread" << std::endl;
            ...
            if (!strcmp(ti->mISerialUSB, ""))
            {
                ...
                ti->connect();
                ti->setWatchdogTime(ti->getWatchdogResetTime()); // reset watchdog
            }
            else
            {
...
static constexpr char GAIN_MODE_Read = 0x0A;
...
// 0x01 = slave
// 0x02 = master
static constexpr char EXTERNAL_SYNC_DISABLED[3] = {0x21, 0x00, 0x00};
static constexpr char EXTERNAL_SYNC_MASTER[3] = {0x21, 0x00, 0x02};
static constexpr char EXTERNAL_SYNC_SLAVE[3] = {0x21, 0x00, 0x01};
...
void TauInterface::setWatchdogTime(unsigned int wait_time)
{
    mWatchdogMutex.lock();

    if (wait_time == 0) // "0" disables watchdog
        mRunWatchdog = false;
    else
        mRunWatchdog = true;

    mWatchdogCntResetValue = wait_time;
    mWatchdogCnt = mWatchdogCntResetValue;

    mWatchdogMutex.unlock();
}

void TauInterface::decrementWatchdogTime()
{
    mWatchdogMutex.lock();
...
unsigned int TauInterface::getWatchdogTime()
{
    unsigned int ret = 1;
...
unsigned int TauInterface::getWatchdogResetTime()
{
    unsigned int ret = 1;

    mWatchdogMutex.lock();
    ret = mWatchdogCntResetValue;
    mWatchdogMutex.unlock();

    return ret;
}

bool TauInterface::getWatchdogState()
{
    bool ret = false;
...
TauInterface::TauInterface(callbackTauRawBitmapUpdate cb, void* caller) : mVideoProcessingEnabled(false)
{
    ...
    _gainMode = thermal_grabber::HighGain;

    mWatchdogCnt = 10;
    mWatchdogCntResetValue = 10;
    mRunWatchdog = true;
...
{
    setWatchdogTime(0); // stop watchdog thread

    if (threadWatchdog.joinable())
    {
        threadWatchdog.join();
        std::cout << "joining watchdog" << std::endl;
    }
    std::cout << "watchdog closed" << std::endl;
...
bool TauInterface::checkSettings()
{
    ...
    // after reconnect, so clear old infos.
    mVideoProcessingEnabled = false;
    ...
    mDigitalOutputEnabledChecked = false;
    mDigitalOutputXPMode14bitChecked = false;
    mDigitalOutputCMOSBitDepth14bitChecked = false;
    mTLinearEnabledChecked = false;
    ...
    */
    ...
    case 0x0A:
        ...
        if (buffer[9] == 0x00)
        {
            ...
            _gainMode = thermal_grabber::GainMode::Automatic;
        }
        else if (buffer[9] == 0x01)
        {
            std::cout << "Low gain" << std::endl;
            _gainMode = thermal_grabber::GainMode::LowGain;
        }
        else if (buffer[9] == 0x02)
        {
            std::cout << "High gain" << std::endl;
            _gainMode = thermal_grabber::GainMode::HighGain;
        }
        else if (buffer[9] == 0x03)
        {
            std::cout << "Manual" << std::endl;
            _gainMode = thermal_grabber::GainMode::Manual;
        }
        break;
    ...
    case 0x12:
        ...
        break;
    case 0x20:
        std::cout << "response READ_SENSOR" << std::endl;
        
        // Reply has 2 bytes at index 8 and 9 of buffer.

        if (buffer[9] & 0x01)
        {
            std::cout << "Bit0 set: Outside temp range!" << std::endl;
        }
        else
        {    
            std::cout << "Bit0 not set: Inside temp range!" << std::endl;
        }
    case 0x21:
        if (buffer[8] == 0x00)
        {
            if (buffer[9] == 0x00)
                std::cout << "External Sync set to disabled" << std::endl;
            else if (buffer[9] == 0x01)
                std::cout << "External Sync set to slave" << std::endl;
            else if (buffer[9] = 0x02)
                std::cout << "External Sync set to master" << std::endl;
            else
                std::cout << "External Sync value unknown?!" << std::endl;
        }
        else
            std::cout << "External Sync value unknown?!" << std::endl;
        break;
    ...
    case 0x13:
        ...
        if (buffer[9] == 0x00) std::cout << "Plateau histogram" << std::endl;
        else if (buffer[9] == 0x01) std::cout << "Once bright" << std::endl;
        else if (buffer[9] == 0x02) std::cout << "Auto bright" << std::endl;
        else if (buffer[9] == 0x03) std::cout << "Manual" << std::endl;
        else if (buffer[9] == 0x04) std::cout << "Not defined" << std::endl;
        else if (buffer[9] == 0x05) std::cout << "Linear AGC" << std::endl;
    ...
    case 0x14:
        ...
        std::cout << (int)(char)buffer[9] << std::endl;
        
        break;
    case 0x15:
        std::cout << "Brightness: ";
        
        // contrast value (0-16383)
        std::cout << (int)(char)buffer[9] << std::endl;
...
void TauInterface::processVideoData(uint16_t* buffer, uint32_t size)
{
    ...
    if (mVideoProcessingEnabled)
    {
        ...
        unsigned int resetTime = getWatchdogResetTime();
        setWatchdogTime(resetTime);
    }
}
...
thermal_grabber::GainMode TauInterface::getGainMode()
{
    sendCommand(GAIN_MODE_Read, 0, 0);
    return _gainMode;
}
...
void TauInterface::disableExternalSynchronization()
{
    setSyncModeNoExternalSyncOrMaster();
}

void TauInterface::enableExternalSynchronizationSlaveMode()
{
    setSyncModeCameraSlave();
}

void TauInterface::enableExternalSynchronizationMasterMode(uint32_t freq, uint32_t count)
{
    setSyncModeGrabberMaster(freq, count);
}

void TauInterface::setExternalSyncToDisabled()
{
    sendCommand(EXTERNAL_SYNC_DISABLED[0],
            const_cast<char*>(&EXTERNAL_SYNC_DISABLED[1]),
            sizeof(EXTERNAL_SYNC_DISABLED)-1);
}

void TauInterface::setExternalSyncToMaster()
{
    sendCommand(EXTERNAL_SYNC_MASTER[0],
            const_cast<char*>(&EXTERNAL_SYNC_MASTER[1]),
            sizeof(EXTERNAL_SYNC_MASTER)-1);
}

void TauInterface::setExternalSyncToSlave()
{
    sendCommand(EXTERNAL_SYNC_SLAVE[0],
            const_cast<char*>(&EXTERNAL_SYNC_SLAVE[1]),
            sizeof(EXTERNAL_SYNC_SLAVE)-1);
}
